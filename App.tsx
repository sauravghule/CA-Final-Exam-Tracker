...
import { ChevronDown, Check, Plus, Trash2, AlertTriangle, Search, X, Download, Upload, ShieldCheck } from 'lucide-react';
...
function DashboardView({ chapters, customChapters, settings, onSettingsChange, onJump, onReset, onExport, onImport }) {
  ...
  return (
    <div className="px-4 py-3">
      ...
      <div className="font-semibold mb-2" style={{ fontSize: 14, color: 'var(--navy)' }}>Backup & Restore</div>
      <div className="cft-card p-3 mb-4">
        <div className="flex items-start gap-2 mb-3">
          <ShieldCheck size={18} style={{ color: 'var(--green)', flexShrink: 0, marginTop: 2 }} />
          <div style={{ fontSize: 12, color: 'var(--ink-soft)' }}>Your study data is stored locally on this iPad. Export a backup regularly before clearing browser data or moving devices.</div>
        </div>
        <div className="flex gap-2 flex-wrap">
          <button onClick={onExport} className="flex items-center gap-1 px-3 py-2 rounded-md" style={{ fontSize: 13, background: 'var(--navy)', color: '#fff' }}><Download size={14} /> Export backup</button>
          <label className="flex items-center gap-1 px-3 py-2 rounded-md cursor-pointer" style={{ fontSize: 13, border: '1px solid var(--rule)', color: 'var(--ink)' }}>
            <Upload size={14} /> Import backup
            <input type="file" accept="application/json,.json" className="hidden" onChange={async (e) => { const file = e.target.files?.[0]; if (file) await onImport(file); e.target.value = ''; }} />
          </label>
        </div>
      </div>
    ...
    <div className="cft-root" style={{ minHeight: '100vh', paddingBottom: 32 }}>
      ...
      {activeTab === 'dashboard' && (
        <DashboardView chapters={chapters} customChapters={customChapters} settings={settings}
          ...
          onExport={() => exportBackup({ chapters, customChapters, dailyLogs, mockTests, settings })}
          onImport={async (file) => {
            const data = await importBackup(file);
            if (!data) return;
            if (data.chapters) setChapters(data.chapters);
            if (data.customChapters) setCustomChapters(data.customChapters);
            if (data.dailyLogs) setDailyLogs(data.dailyLogs);
            if (data.mockTests) setMockTests(data.mockTests);
            if (data.settings) setSettings({ ...DEFAULT_SETTINGS, ...data.settings });
            await Promise.all([
              persist('chapters-v1', data.chapters || {}),
              persist('custom-chapters-v1', data.customChapters || {}),
              persist('daily-logs-v1', data.dailyLogs || {}),
              persist('mock-tests-v1', data.mockTests || DEFAULT_MOCKS),
              persist('settings-v1', { ...DEFAULT_SETTINGS, ...(data.settings || {}) }),
            ]);
          }} />
      )}
      {activeTab === 'today' && (
